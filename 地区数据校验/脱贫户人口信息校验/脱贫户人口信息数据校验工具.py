...
class DataHandle:
    def __init__(self):
        ...
        self.sqlDictionary = {
            ...
            """,
            '29-不同户但户联系电话号码一致': 'select a.* from tables as a left join (select 户联系电话 from tables where "与户主关系"="户主" group by 户联系电话 having count(*) > 1) as b on a.户联系电话 = b.户联系电话 where a.户联系电话 = b.户联系电话 and a.与户主关系 ="户主" order by a.户联系电话',
            '30-脱贫户家庭成员义务教育适龄儿童不在校，未填写失学或辍学原因的（出生日期小于2015年9月;出生日期大于2006年9月）': 'select * from tables where ((substr("证件号码",7,6) + 0) < 201509) and ((substr("证件号码",7,6) + 0) > 200609) and length("在校生状况") = 0 and length("失学或辍学原因") = 0;',
            '31-脱贫户对象8岁以上健康人口为学龄前儿童或学前教育': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) > 8) and 健康状况 = "健康" and 在校生状况 in ("学龄前儿童","学前教育");',
            '32-脱贫户和监测对象人员义务教育阶段失学或辍学原因“因病”或“因残”不在校儿童健康状况为“健康”出生日期小于2015年9月;出生日期大于2006年9月）': """
            select * from tables where ((substr("证件号码",7,6) + 0) < 201509) and ((substr("证件号码",7,6) + 0) > 200609) and length("在校生状况") = 0 and 健康状况 ="健康" and  (失学或辍学原因 like "%病%" or 失学或辍学原因 like "%残%");
            """,
            '33-脱贫户和监测对象15周岁（不含）以下健康人口有文化程度（提前毕业等原因除外）': 'select * from tables where length("文化程度") <> 0 and 健康状况 = "健康" and (2021 - (substr("证件号码",7,4) + 0) < 15);',
            '34-有务工区域但务工时间不是1-12月之间的数字': 'select * from tables where length("务工所在地") <> 0 and ("务工时间（月）" + 0 not between 0.5 and 12);',
            '35-脱贫户和监测对象人员享受公益性岗位但务工地点为空': 'select * from tables where length("公益性岗位类型") <> 0 and length("务工所在地") = 0;',
            '36-脱贫户和监测对象人员在校生有务工地点': 'select * from tables where length("在校生状况") > 0 and length("务工所在地") > 0;',
            '37-脱贫户和监测对象人员填写了聘用公益性岗位但未填写公益性岗位(月数)': 'select * from tables where length("公益性岗位类型") > 0 and (length("公益性岗位(月数)") = 0 or "公益性岗位(月数)" = 0);',
            '38-脱贫户和监测对象人员填写了公益性岗位(月数)但未填写聘用公益性岗位': 'select * from tables where length("公益性岗位类型") = 0 and "公益性岗位(月数)" + 0 > 0;',
            '39-脱贫户和监测对象人员无（丧失）劳动力有务工地点': 'select * from tables where 劳动技能 in ("丧失劳动力","无劳动力") and length("务工所在地") > 0;',
            '40-13岁以上健康脱贫户和监测对象人员在校生状况为小学、学前教育、学龄前儿童': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) > 13) and 在校生状况 in ("学龄前儿童","学前教育","小学");',
            '41-脱贫户和监测对象人口有务工地点但务工企业为空': 'select * from tables where length("务工所在地") > 0 and length("务工企业名称") = 0;',
            '42-脱贫户和监测对象人口务工地点未填报到县': 'select * from tables where length("务工所在地") > 0 and ("务工所在地" not like "%市%" and "务工所在地" not like "%县%" and "务工所在地" not like "%团%");',
            '43-脱贫户和监测对象家庭成员与户主关系3人及以上为其他': 'select a.* from tables as a left join (select 户编号 from tables where "与户主关系"="其他" group by 户编号 having count(*) >= 3) as b on a.户编号 = b.户编号 where a.户编号 = b.户编号 order by 户编号',
            '44-脱贫户和监测对象为特困供养人员为18（含）-60（不含）健康非在校生': 'select * from tables where 是否特困供养人员 = "是" and (2021 - (substr("证件号码",7,4) + 0) >= 18 and 2021 - (substr("证件号码",7,4) + 0) < 60) and 健康状况 = "健康" and length("在校生状况") = 0;',
            '45-脱贫户和监测对象家庭成员务工时间小于公益性岗位聘用月数': 'select * from tables where "公益性岗位(月数)" + 0 > "务工时间（月）" + 0;',
            '46-脱贫户和监测对象家庭成员30岁以上在校生状况不为空': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) > 30) and length("在校生状况") > 0;',
            '47-脱贫户和监测对象家庭成员16（不含）岁以上健康人员在校生状况为初中': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) > 16) and "在校生状况" = "初中" and "健康状况" = "健康";',
            '48-脱贫户和监测对象家庭成员15(不含)岁以下人员上在校生状况为高中及以上': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) < 15) and "在校生状况" not in ("学龄前儿童","学前教育","小学","七年级","八年级","九年级","");',
            '49-脱贫户和监测对象家庭成员务工时间不为数字': 'select * from tables where length("务工时间（月）") > 0 and "务工时间（月）" not regexp "(^[0-9]+.[0-9]+$)|(^[0-9]+$)";',
            '50-脱贫人口残疾人证号不符合校验规则': 'select * from tables where ((SUBSTR("证件号码",19,1) + 0 ) not in (1,2,3,4,5,6,7) or (SUBSTR("证件号码",20,1) + 0 ) not in (1,2,3,4)) and length("证件号码") >=20;',
            '51-脱贫人口“民族”为空': 'select * from tables where length("民族") = 0;',
            '52-脱贫人口“与户主关系”与“性别”指标值逻辑关系错误': """
            select * from tables where ((SUBSTR("证件号码",17,1) + 0) in (0,2,4,6,8) and "与户主关系" in ( "之子","之女婿","之孙子","之外孙子","之父","之岳父","之公公","之祖父","之外祖父","之曾孙子","之侄儿","之叔伯")) or 
            (SUBSTR("证件号码",17,1) + 0) in (1,3,5,7,9) and "与户主关系" in ("之女","之儿媳","之孙女","之外孙女","之母","之岳母","之婆婆","之祖母","之外祖母","之曾孙女","之侄女","之兄弟媳妇");
            """,
            '53-16（含）-60周岁（不含）健康脱贫人口劳动能力为丧失劳动力': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) >= 16) and (2021 - (substr("证件号码",7,4) + 0) < 60) and "健康状况" = "健康" and "劳动技能" = "丧失劳动力";',
            '54-16（含）-60周岁（不含）健康非在校生脱贫人口无劳动能力、丧失、弱劳': 'select * from tables where ((2021 - (substr("证件号码",7,4) + 0) >= 16) and (2021 - (substr("证件号码",7,4) + 0) < 60)) and length("在校生状况") = 0 and "健康状况" = "健康" and "劳动技能" in ("弱劳动力或半劳动力","丧失劳动力","无劳动力");',
            '55-16岁（不含）以下脱贫人口劳动能力不是无劳动能力': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) < 16) and "劳动技能" <> "无劳动力";',
            '56-已外出务工或计划外出务工的脱贫人口无（含丧失）劳动能力': 'select * from tables where length("务工所在地") > 0 and "劳动技能" in ("无劳动力","丧失劳动力");',
            '57-脱贫人口“文化程度”、“在校生状况”和“失学或辍学原因”同时为空': 'select * from tables where length("文化程度") = 0 and length("在校生状况") = 0 and length("失学或辍学原因") = 0;',
            '58-脱贫人口参加了公益性岗位，但聘用月数非0-12之间的数字': 'select * from tables where length("公益性岗位类型") > 0 and ("公益性岗位(月数)" + 0 not between 0 and 12)',
            '59-脱贫人口未参加城乡居民（城镇职工）基本医疗保险，但“是否接受大病医疗救助”选项为是': 'select * from tables where "是否参加新型农村合作医疗（城乡居民基本医疗保险）" = "否" and "是否接受大病医疗救助" = "是" and "是否参加城镇职工基本医疗保险" = "否";',
            '60-16岁以下脱贫人口参加公益性岗位': 'select * from tables where (2021 - (substr("证件号码",7,4) + 0) < 16) and length("公益性岗位类型") > 0;',
        }
